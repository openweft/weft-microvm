...
common `ncl-init` kernel over virtio-fs. This is the runtime behind the
`weft microvm` CLI; it is **not** the classic full-VM path (boot.iso + cloud-init).

Extracted from the former standalone `ncl` runner (nano-container-linux) so the
logic lives in the openweft org and is consumed directly by the single `weft`
binary — there is no separate `ncl` binary anymore.
...
- `Pull` an OCI image → rootfs extracted to `$XDG_DATA_HOME/weft-microvm/images/<refsafe>/rootfs/`,
  with `.ncl/config.json` materialised from the image config.
- `Run` boots a microVM by dialing the weft agent's gRPC API and calling
  ...
  (mount tag `rootfs0`); the kernel/init is the **shared** `ncl-init` UKI, not a
...
// Pull caches an OCI image: pull + extract rootfs + write .ncl/config.json.
...
type Args struct {
	...
	VzdSocket string   // weft agent gRPC socket (default ~/.vzd/vzd.sock)
...
```go
// PackToFile packs a Linux ncl-init ELF into an initramfs cpio.gz.
...
- [`openweft/weft-microvm-init`](../weft-microvm-init) — guest-side `ncl-init` / pod supervisor.
