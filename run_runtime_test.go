...
func TestLocateBootArtefacts_MissingErrorIsHelpful(t *testing.T) {
	...
	msg := err.Error()
	for _, expected := range []string{"WEFT_KERNEL", "WEFT_INIT_ISO", "direct-Linux"} {
...
func TestLocateBootArtefacts_ISOFromXDGDataHome(t *testing.T) {
	...
	iso := filepath.Join(tmp, "weft-microvm", "weft-microvm-init.iso")
...
func TestLocateBootArtefacts_DirectLinuxFromEnv(t *testing.T) {
	...
	t.Setenv("WEFT_KERNEL", kernel)
	t.Setenv("WEFT_INITRD", initrd)
...
func TestLocateBootArtefacts_DirectLinuxPrefersOverISO(t *testing.T) {
	...
	microvmDir := filepath.Join(tmp, "weft-microvm")
	if err := os.MkdirAll(microvmDir, 0o755); err != nil {
		t.Fatal(err)
	}
	// Both artefact sets live at the defaults — the kernel one wins.
	for _, p := range []string{filepath.Join(microvmDir, "kernel"), filepath.Join(microvmDir, "initrd"), filepath.Join(microvmDir, "weft-microvm-init.iso")} {
		if err := os.WriteFile(p, []byte("x"), 0o644); err != nil {
			t.Fatal(err)
		}
	}
...
func TestApplyRunOverrides_NoCmd_LeavesConfigAlone(t *testing.T) {
	...
	cfgPath := filepath.Join(microvmDir, "config.json")
...
func TestApplyRunOverrides_MissingConfig_Errors(t *testing.T) {
	...
	if err := applyRunOverrides(rootfs, Args{Image: "x:1", Cmd: []string{"sh"}}); err == nil {
		t.Fatal("expected error when .weft-microvm/config.json absent")
	}
}
...
func TestApplyRunOverrides_BadJSON_Errors(t *testing.T) {
	...
	if err := os.WriteFile(filepath.Join(microvmDir, "config.json"), []byte("{not json"), 0o644); err != nil {
		t.Fatal(err)
	}
...
func TestApplyRunOverrides_WriteError(t *testing.T) {
	...
	// Make the .weft-microvm dir read-only so the rewrite (WriteFile with
	...
	if err := os.Chmod(microvmDir, 0o500); err != nil {
		t.Fatal(err)
	}
	t.Cleanup(func() { _ = os.Chmod(microvmDir, 0o755) })
	err := applyRunOverrides(rootfs, Args{Image: "x:1", Cmd: []string{"sh"}})
